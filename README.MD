...
| PUBLIC_FRONT_URL            | No       | Frontend URL in this format `http://your.public.ip`                                                                                                                                                                                                                                                               | http://localhost |
...
| ENABLE_SSO                  | No       | Enables or disables SSO. (`true` or `false` ) Requires `OAUTH2_PROVIDER`, `OAUTH2_CLIENT_ID` and `OAUTH2_CLIENT_SECRET`. Make sure to add `PUBLIC_FRONT_URL` as authorized origin and `${PUBLIC_FRONT_URL}/api/oauth2/callback/${OAUTH2_PROVIDER}` as authorized redirection URI in your Oauth2 provider settings | false            |
...
For production deployments, it’s strongly recommended to use unique credentials and not rely on defaults. You should also [setup TLS](./dev-docs/Set%20up%20TLS.md)
...
your custom backend server url: `${PUBLIC_FRONT_URL}/api`
