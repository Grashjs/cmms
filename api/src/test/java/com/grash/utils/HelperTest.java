...
class HelperTest {
    ...
    class ExtractClientIp {
        ...
        @Test
        void xForwardedFor_returnsFirstIp() {
            when(request.getHeader("X-Forwarded-For")).thenReturn("192.168.1.1, 10.0.0.1");
            assertEquals("192.168.1.1", Helper.extractClientIp(request));
        }

        @Test
        void xRealIp_returnsIt() {
            when(request.getHeader("X-Forwarded-For")).thenReturn(null);
            when(request.getHeader("X-Real-IP")).thenReturn("10.0.0.5");
            assertEquals("10.0.0.5", Helper.extractClientIp(request));
        }

        @Test
        void cloudflareHeader_returnsIt() {
            when(request.getHeader("X-Forwarded-For")).thenReturn(null);
            when(request.getHeader("X-Real-IP")).thenReturn(null);
            when(request.getHeader("CF-Connecting-IP")).thenReturn("203.0.113.1");
            assertEquals("203.0.113.1", Helper.extractClientIp(request));
        }

        @Test
        void trueClientIp_returnsIt() {
            when(request.getHeader("X-Forwarded-For")).thenReturn(null);
            when(request.getHeader("X-Real-IP")).thenReturn(null);
            when(request.getHeader("CF-Connecting-IP")).thenReturn(null);
            when(request.getHeader("True-Client-IP")).thenReturn("198.51.100.1");
            assertEquals("198.51.100.1", Helper.extractClientIp(request));
        }

        @Test
        void unknownHeader_skipsIt() {
            when(request.getHeader("X-Forwarded-For")).thenReturn("unknown");
            when(request.getHeader("X-Real-IP")).thenReturn(null);
            when(request.getHeader("CF-Connecting-IP")).thenReturn(null);
            when(request.getHeader("True-Client-IP")).thenReturn(null);
            when(request.getRemoteAddr()).thenReturn("10.0.0.1");
            assertEquals("10.0.0.1", Helper.extractClientIp(request));
        }
        ...
        void noHeaders_fallsBackToRemoteAddr() {
            ...
            assertEquals("192.168.1.100", Helper.extractClientIp(request));
        }

        @Test
        void noHeadersAndNoRemoteAddr_returnsUnknown() {
            when(request.getHeader("X-Forwarded-For")).thenReturn(null);
            when(request.getHeader("X-Real-IP")).thenReturn(null);
            when(request.getHeader("CF-Connecting-IP")).thenReturn(null);
            when(request.getHeader("True-Client-IP")).thenReturn(null);
            when(request.getRemoteAddr()).thenReturn(null);
            assertEquals("unknown", Helper.extractClientIp(request));
        }
    }
